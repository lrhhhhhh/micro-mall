...
func (m *defaultStockModel) Insert(ctx context.Context, data *Stock) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.GoodsId, data.Count)
	return ret, err
}

func (m *defaultStockModel) Update(ctx context.Context, data *Stock) error {
	query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, stockRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, data.GoodsId, data.Count, data.Id)
	return err
}
